...
import { auth, db } from './firebase-config.js?v=2.3.9';
import { onAuthStateChanged, signInAnonymously } from "firebase/auth";
import { doc, onSnapshot } from "firebase/firestore";
// Main modules are now lazy-loaded
import { getPermisosUsuario, getSystemVersion, migrateConductoresToPublicadores } from './data/firestore-services.js?v=2.3.9';
import { showNotification } from './modules/utils/helpers.js?v=2.3.9';
import { initPWA } from './modules/utils/pwa-manager.js?v=2.3.9';
import { initTheme, createThemeToggle } from './modules/utils/theme-manager.js?v=2.3.9';
...
async function loadLogin() {
    if (!ModuleCache.login) ModuleCache.login = await import('./modules/login.js?v=2.3.9');
    return ModuleCache.login.renderLogin;
}

async function loadAdmin() {
    if (!ModuleCache.admin) ModuleCache.admin = await import('./modules/admin-dashboard.js?v=2.3.9');
    return ModuleCache.admin.renderAdminDashboard;
}

async function loadConductor() {
    if (!ModuleCache.conductor) ModuleCache.conductor = await import('./modules/conductor-dashboard.js?v=2.3.9');
    return ModuleCache.conductor.renderConductorDashboard;
}

// --- FORCED ONE-TIME SYNC TO v2.3 ---
(async () => {
    const SYNC_VERSION = '2.3.9';
