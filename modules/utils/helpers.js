...
export const showNotification = (
    ...
    if (isSync) {
        const existing = Array.from(hud.children).find(
            (c) => c.innerHTML.includes("WORKFLOW") || c.innerHTML.includes("Sinc:"),
        );
...
export const completeSyncNotification = (moduleName) => {
    ...
    if (card) {
        card.className =
            "bg-white/90 dark:bg-slate-900/90 border-emerald-100/50 dark:border-emerald-500/10 border backdrop-blur-xl px-6 py-4.5 rounded-[2rem] shadow-[0_20px_50px_rgba(16,185,129,0.15)] flex flex-col gap-1.5 animate-slide-left pointer-events-auto transform transition-all duration-700";
        ...
        if (iconWrap) {
            iconWrap.className =
                "w-11 h-11 bg-emerald-50 dark:bg-emerald-500/10 rounded-2xl flex items-center justify-center text-emerald-500 dark:text-emerald-400 shadow-sm scale-105 transition-transform duration-350";
        ...
        if (label) {
            ...
            label.className =
                "text-[9px] font-black text-emerald-500 dark:text-emerald-400 uppercase tracking-[0.25em]";
        }
        const title = card.querySelector("h4");
        if (title) {
            title.innerText = `${moduleName} al día`;
            title.className =
                "text-xs font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-tight leading-none";
        }
...
export const formatGroups = (val) => {
    ...
    const clean = val.replace(/grupos?/gi, "").trim();
