...
import { showCustomConfirm, showModal, UIHelpers } from "../services/ui-helpers.js";
...
export const renderHistorialView = async (container) => {
    ...
    if (currentProg?.dias) {
        currentProg.dias.forEach((d) => {
            ["manana", "tarde", "noche", "zoom"].forEach((turn) => {
                if (d[turn]?.territorio) {
                    String(d[turn].territorio)
                        .split(/[,;/]/)
                        .forEach((n) => {
    ...
    if (syncBtn) {
        syncBtn.onclick = async () => {
            ...
            try {
                await runSystemDiagnosticsAndRepair((msg, _pc) => {
    ...
    const renderGrid = () => {
        ...
        const displayList = allTerritorios
            .filter((t) => {
