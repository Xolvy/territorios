import { addRecurso, deleteRecurso, getRecursos, updateRecurso } from "../../data/firestore-services.js";
import { showCustomConfirm, showModal } from "../services/ui-helpers.js";
import { showNotification } from "../utils/helpers.js";

export const renderRecursosTab = async (container, _configData = null, _appVersion = null) => {
            ...
            <section class="space-y-10">
                <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
                    <div>
                        ...
                        <p class="text-[10px] text-slate-600 dark:text-slate-400 font-bold uppercase tracking-[0.3em] mt-2 ml-1">Recursos digitales para el ministerio</p>
                    </div>
                    ...
                    `
                        ...
                        <div class="modern-card group !p-0 overflow-hidden border-slate-100 dark:border-white/5 flex flex-col shadow-xl hover:shadow-2xl transition-all hover:border-primary/30">
                            <div class="h-44 bg-slate-100 dark:bg-black/40 relative overflow-hidden">
                                ${
                                    r.imagen
                                        ? `<img src="${r.imagen}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">`
                            ...
                            <div class="p-6 flex-1 min-w-0 flex flex-col space-y-4">
        ...
        showModal(
            ...
            <div class="flex flex-col h-full bg-white dark:bg-[#0a0f18] rounded-[2.5rem] overflow-hidden">
                ...
                <div class="flex-1 min-w-0 overflow-y-auto custom-scrollbar p-10 space-y-8 bg-slate-50 dark:bg-black/20">
                    <div class="space-y-6">
                        <div class="space-y-3">
                            <label class="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest ml-1 block">Título del Material</label>
                        ...
                        <div class="space-y-3">
                            <label class="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest ml-1 block">Enlace (URL)</label>
                        ...
                        <div class="space-y-3">
                            <label class="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest ml-1 block">Miniatura (URL Opcional)</label>
                        ...
                        <div class="space-y-3">
                            <label class="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest ml-1 block">Descripción breve</label>
                ...
                <footer class="shrink-0 p-8 bg-white dark:bg-black/40 border-t border-slate-100 dark:border-white/5 flex gap-4">
                    <button id="btn-cancel-rec" class="btn-pro flex-1 min-w-0 py-5 bg-slate-50 dark:bg-white/5 hover:bg-slate-100 dark:hover:bg-white/10 text-slate-600 dark:text-slate-400 font-black rounded-2xl text-[10px] uppercase tracking-[0.2em] border border-slate-200 dark:border-white/10 transition-all active:scale-95">
                        Cancelar
                    </button>
                    <button id="save-rec-btn" class="btn-pro flex-[1.5] py-5 bg-primary hover:bg-primary-light text-white font-black rounded-2xl text-[10px] uppercase tracking-[0.2em] shadow-xl shadow-primary/20 transition-all active:scale-95 flex items-center justify-center gap-2">
    ...
    window.deleteRecurso = (id) =>
        showCustomConfirm("¿Eliminar este material de apoyo permanentemente?", async () => {
