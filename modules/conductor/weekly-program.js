...
export const initializeWeeklyProgram = (
    container,
    userMods,
    allTerritorios,
    territoryMap,
    ...
    const _currentWeek =
        typeof currentWeekStart === "string" && currentWeekStart.includes("-")
    ...
    const loadWeekData = async () => {
        ...
        try {
                ...
                const hasActivity =
                    ...
                    shifts.some((s) => {
                        ...
                        return (
                            sd &&
                            sd.enabled !== false &&
                            (sd.conductor || sd.lugar || sd.hora || sd.territorio || sd.faceta)
                        );
                    });
            ...
            if (programCardsContainer) {
                programCardsContainer.innerHTML =
                    '<p class="text-center text-rose-500 font-bold p-10">Error al cargar la programación</p>';
            }
        }
    };
    ...
    const renderFilters = () => {
        ...
        turnFilters.innerHTML = turnosArr
            .map((t) => {
    ...
    window.toggleProgTurn = (id) => {
        ...
        renderFilters();
        renderFullProgramaCards(
            window._globalPrograma,
    ...
    window.showTodayProg = () => {
        ...
        tabs.forEach((tab) => {
            ...
            if (text === nombreHoy.toLowerCase() || text.startsWith(nombreHoy.substring(0, 3).toLowerCase())) {
                tab.click();
                tabEncontrado = true;
            }
        });
        ...
        if (!tabEncontrado) {
            ...
            const tabTodos = Array.from(tabsAll).find(
                (t) => t.textContent.includes("Toda") || t.textContent.includes("Todos"),
            );
