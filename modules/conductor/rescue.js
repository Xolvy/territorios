...
export const renderRescueMissions = (allTerritorios, normalizedName, myExtraMissions, rescueCandidates, totalMissionCount, onAsumir) => {
            ...
            <div class="flex flex-col h-full bg-white dark:bg-[#0a0f18] rounded-[3rem] overflow-hidden">
                <header class="shrink-0 bg-rose-600 p-8 text-white relative overflow-hidden">
                    ...
                    <div class="flex items-center gap-6 relative z-10">
                        ...
                        <div class="space-y-1">
                            <h3 class="text-2xl font-black uppercase tracking-tighter">Centro de Rescate</h3>
                            <p class="text-[10px] font-black uppercase tracking-[0.2em] opacity-80">Territorios con Alertas de Tiempo</p>
                        </div>
                    </div>
                </header>
                ...
                <div class="flex-1 overflow-y-auto custom-scrollbar p-10 space-y-10 bg-slate-50 dark:bg-black/20">
                    ...
                    <section class="space-y-4">
                        <h4 class="text-[10px] font-black uppercase text-slate-400 tracking-[0.4em] ml-2">Mis Misiones Activas</h4>
                        <div class="grid grid-cols-1 gap-4">
                            ${myExtraMissions.map(t => `
                                <div class="modern-card !p-5 bg-white dark:bg-white/5 border-l-4 border-emerald-500 shadow-sm flex justify-between items-center group">
                                    <div class="flex items-center gap-4">
                                        <span class="w-12 h-12 bg-emerald-500/10 text-emerald-600 rounded-xl flex items-center justify-center font-black text-lg">T${t.numero}</span>
                                        <div>
                                    ...
                                    <button onclick="window.viewMapFromReport('${t.id}')" class="w-10 h-10 bg-slate-100 dark:bg-white/5 text-slate-400 hover:text-emerald-500 rounded-xl transition-colors">
                    ...
                    <section class="space-y-4">
                        ...
                        <div class="text-center py-12 px-8 bg-white dark:bg-white/5 rounded-[2.5rem] border-2 border-dashed border-slate-200 dark:border-white/10 opacity-60">
                            ...
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-relaxed">¡Increíble! Todos los territorios están siendo atendidos a tiempo.</p>
                        </div>
                        ` : `
                            <div class="grid grid-cols-1 gap-6">
                                ${rescueCandidates.map(t => {
            const delayH = Math.floor((Date.now() - t.lastAssignmentChange) / (1000 * 60 * 60));
            const isFree = t.estado === 'Disponible';
            const accent = isFree ? 'indigo' : 'rose';
            const name = isFree ? 'TERRITORIO LIBRE' : (t.asignado_a || '—');
            ...
            return `
                                    <div class="modern-card !p-8 bg-white dark:bg-white/5 border border-slate-100 dark:border-white/5 shadow-xl hover:shadow-2xl transition-all group overflow-hidden relative">
                                        <div class="absolute top-0 right-0 p-8 opacity-5 text-6xl grayscale">
                                            <i class="fas ${isFree ? 'fa-flag' : 'fa-clock'}"></i>
                                        </div>
                                        <div class="flex justify-between items-start mb-6">
                                            <div class="flex items-center gap-5">
                                                ...
                                                <div class="space-y-1">
                                                    <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest">${isFree ? 'ESTADO' : 'RESPONSABLE'}</p>
                                                    <p class="text-sm font-black text-slate-800 dark:text-white uppercase truncate">${name}</p>
                                                </div>
                                            </div>
                                            <div class="text-right">
                                                <span class="px-3 py-1 bg-${accent}-500/10 text-${accent}-600 rounded-lg text-[8px] font-black uppercase tracking-widest border border-${accent}-500/10">
                                                    ${isFree ? 'DISPONIBLE' : `DEMORA: ${delayH}H`}
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        <div class="bg-slate-50 dark:bg-black/20 p-5 rounded-2xl border border-slate-100 dark:border-white/5 mb-8">
                                            <p class="text-[10px] text-slate-500 dark:text-slate-400 font-bold leading-relaxed uppercase tracking-tight">
                                                ...
                                                ${formatManzanas(t.manzanas) || 'Territorio disponible.'}
                                            </p>
                                        </div>

                                        <button onclick="window.handleRescueTerritory('${t.id}', '${t.numero}', '${name.replace(/'/g, "\\\'")}', '${(t.manzanas || '').replace(/'/g, "\\\'")}', ${isFree})"
                                                class="w-full py-5 bg-${accent}-600 hover:bg-${accent}-700 text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.3em] shadow-lg shadow-${accent}-600/20 transition-all active:scale-95 flex items-center justify-center gap-3">
                                            <i class="fas ${isFree ? 'fa-plus' : 'fa-handshake-angle'}"></i> 
    ...
    const rescueBtnClass = totalMissionCount > 0
        ? 'bg-rose-600 border-rose-500 text-white hover:bg-rose-700 active:scale-95 shadow-rose-600/20'
        : 'bg-white/10 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-white/10 hover:bg-white/20';

    return `
        <div class="flex items-center gap-4 bg-slate-50 dark:bg-white/5 p-4 rounded-[2.5rem] border border-slate-100 dark:border-white/5 shadow-inner">
