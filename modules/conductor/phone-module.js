...
import { updateTelefonoStatus, updateTelefono } from '../../data/firestore-services.js';
...
const COLOR_CLASSES = {
    emerald: { bg: 'bg-emerald-500/10 dark:bg-emerald-500/15', border: 'border-emerald-500/25', text: 'text-emerald-600 dark:text-emerald-400', solid: 'bg-emerald-500', hover: 'hover:bg-emerald-500/20' },
    rose:    { bg: 'bg-rose-500/10 dark:bg-rose-500/15',       border: 'border-rose-500/25',    text: 'text-rose-600 dark:text-rose-400',       solid: 'bg-rose-500',    hover: 'hover:bg-rose-500/20'    },
    amber:   { bg: 'bg-amber-500/10 dark:bg-amber-500/15',     border: 'border-amber-500/25',   text: 'text-amber-600 dark:text-amber-400',     solid: 'bg-amber-500',   hover: 'hover:bg-amber-500/20'   },
    indigo:  { bg: 'bg-indigo-500/10 dark:bg-indigo-500/15',   border: 'border-indigo-500/25',  text: 'text-indigo-600 dark:text-indigo-400',   solid: 'bg-indigo-500',  hover: 'hover:bg-indigo-500/20'  },
    slate:   { bg: 'bg-slate-500/10 dark:bg-slate-500/15',     border: 'border-slate-500/25',   text: 'text-slate-600 dark:text-slate-400',     solid: 'bg-slate-500',   hover: 'hover:bg-slate-500/20'   },
    red:     { bg: 'bg-red-500/10 dark:bg-red-500/15',         border: 'border-red-500/25',     text: 'text-red-600 dark:text-red-400',         solid: 'bg-red-500',     hover: 'hover:bg-red-500/20'     },
    teal:    { bg: 'bg-teal-500/10 dark:bg-teal-500/15',       border: 'border-teal-500/25',    text: 'text-teal-600 dark:text-teal-400',       solid: 'bg-teal-500',    hover: 'hover:bg-teal-500/20'    },
};
...
export const initializePhoneModule = (initialPhones, publicadores, displayName, tbody, onRefresh) => {
    ...
    let pendingChanges = window.pendingPhoneChanges || {};
    ...
    const render = () => {
        ...
        // ── Estadísticas ────────────────────────────────────
        const stats = { Contestaron: 0, 'No contestan': 0, Colgaron: 0, Revisita: 0, 'No llamar': 0, Suspendido: 0, Testigo: 0 };
        let marcados = 0;

        displayPhones.forEach(p => {
            const e = (p.estado || '').toLowerCase().trim();
            if (['contestaron', 'contestó'].includes(e))              { stats['Contestaron']++;  marcados++; }
            else if (['no contestan', 'no contestó', 'no contestaron'].includes(e)) { stats['No contestan']++; marcados++; }
            else if (e === 'colgaron')                                { stats['Colgaron']++;     marcados++; }
            else if (e === 'revisita')                                { stats['Revisita']++;     marcados++; }
            else if (e === 'no llamar')                               { stats['No llamar']++;    marcados++; }
            else if (e === 'suspendido')                              { stats['Suspendido']++;   marcados++; }
            else if (e === 'testigo')                                 { stats['Testigo']++;      marcados++; }
        });
        ...
        if (statsContainer) {
            ...
            `;
            statsContainer.className = 'sticky top-[60px] lg:top-0 z-30 bg-white/97 dark:bg-[#0b0f19]/97 backdrop-blur-md py-3 px-1 grid grid-cols-3 sm:grid-cols-7 gap-2 border-b border-slate-200 dark:border-white/10 mb-4';
        ...
        // ── Filas de la tabla ────────────────────────────────
        tbody.innerHTML = displayPhones.map((p, idx) => {
            ...
            const strikesHtml = strikes > 0
                ? `<div class="flex items-center gap-1 ml-2" title="${strikes} de 3 intentos fallidos">
                    ${Array.from({ length: 3 }).map((_, i) =>
                        `<span class="w-2 h-2 rounded-full transition-all ${i < strikes
            ...
            // Botón de estado
            const statusBtnClasses = hasStatus && statusColor
            ...
            const statusBtnContent = hasStatus && statusCfg
                    ...
                    <div class="flex flex-row items-center gap-2 sm:gap-4">
                        <div class="flex-1 min-w-0">
                            ...
                            <select data-id="${p.id}" data-action="staff"
                                ...
                                <option value=""></option>
                                ${[...(publicadores || [])]
                                    .sort((a, b) => String(a.nombre || '').trim().localeCompare(String(b.nombre || '').trim(), 'es', { sensitivity: 'base' }))
                                    .map(pub => {
