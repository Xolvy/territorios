...
import {
    ...
    getTelefonos,
    solicitarNumeros,
    takeTerritoryPartial,
    transferTerritory,
    updateTelefonoStatus,
} from "../../data/firestore-services.js";
import { MapViewer } from "../map-viewer.js";
import { ReceptionHub } from "../services/reception-hub.js";
import { showCustomPrompt, showModal, UIHelpers } from "../services/ui-helpers.js";
import { formatPhoneNumber, normalizeRobust, showNotification } from "../utils/helpers.js";

export async function openProgressModal(initialId, _filterIds = null) {
...
export async function handleRescueTerritory(id, num, newConductor, manzanasStr, isFree = false) {
    ...
    if (manzanas.length > 1) {
        const result = await new Promise((resolve) => {
            ...
            document.getElementById("rescue-cancel-partial").onclick = () => {
...
export async function openRevisitasModal(displayName) {
    ...
    const [allPhones, allPubs] = await Promise.all([getTelefonos(true), getPublicadores()]);
    const revisitas = allPhones.filter((p) => p.estado === "Revisita");

    const resolveName = (raw) => {
        const clean = String(raw || "").trim();
        if (!clean) return "Sin asignar";
        const found = allPubs.find(
            (p) => p.id === clean || p.email?.toLowerCase() === clean.toLowerCase() || p.nombre === clean,
        );
...
`;
        ...
        setTimeout(() => {
            const panZoomController = UIHelpers.initImagePanZoom("global-png-map", "png-zoom-container");
