...
export const getTelefonosParaSesion = async (conductorName) => {
    try {
        ...
        return finalData.filter((d) => {
            const hasMatch =
                d.solicitado_por && variations.some((v) => v.toLowerCase() === d.solicitado_por.toLowerCase());
...
export const solicitarNumeros = async (cantidad = 30, conductorName = null) => {
    ...
    try {
        ...
        if (candidateDocs.length < cantidad) {
            ...
            const [snapNoContestan, snapColgaron] = await Promise.all([getDocs(qNoContestan), getDocs(qColgaron)]);
        ...
        const result = await runTransaction(db, async (transaction) => {
            ...
            const docSnaps = await Promise.all(candidateDocs.map((d) => transaction.get(d.ref)));
...
export const checkAndResetTelephoneCycle = async (forceRequested = false) => {
    try {
        ...
        const hayLibres =
            !snapEmpty.empty ||
            !snapSinAsignar.empty ||
            !snapNull.empty ||
            !snapContestaron.empty ||
            !snapNoContestan.empty ||
            !snapColgaron.empty;
...
export const finalizarSesionConCrm = async (conductorName, pendingChanges = {}) => {
    try {
        ...
        docSnaps.forEach((snap, i) => {
            if (snap.exists()) docsMap[ids[i]] = snap;
        });
        ...
        for (const id of ids) {
            ...
            const data = docSnap.data();
            const finalState = change.estado !== undefined ? change.estado : data.estado;
            const finalNotas = change.notas !== undefined ? change.notas : data.notas || data.comentario || "";
            const finalData = {
                estado: finalState,
...
export const releaseUnusedTelefonos = async (userId, globalCleanup = false, force = false, explicitIds = []) => {
    try {
        ...
        if (userId) {
            ...
            const sessionQ = query(phoneCol, where("solicitado_por", "in", variations));
        ...
        if (force && explicitIds.length > 0) {
            ...
            if (forceReleasePromises.length > 0) {
                ...
                console.log(
                    `[Phone Service] 🔓 Red de seguridad: ${forceReleasePromises.length} números liberados por ID explícito.`,
                );
            }
        }
        ...
        if (force) {
            ...
            Promise.all([repararTelefonosData(), checkAndResetTelephoneCycle()])
...
export const repararTelefonosData = async () => {
    try {
        ...
        snap.docs.forEach((d) => {
            ...
            const eStr = String(data.estado || "")
                .toLowerCase()
                .trim();
            const isBlankState =
                !eStr || eStr === "sin asignar" || eStr === "no asignado" || eStr === "disponible" || eStr === "null";
